...
def test_status_page_renders_without_key_leak(client, monkeypatch):
    ...
    # Ensure no real key is shown - placeholder text is OK
    assert "sk_test_ABC123" not in html  # ensure no raw key shown
    assert "sk_live_ABC123" not in html  # ensure no raw key shown
