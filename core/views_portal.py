...
def portal_checkout_finalize(request):
    ...
    audit_emit(
        ...
        message=f"Portal booking (checkout flow) created by client {client.id}",
        actor=request.user,
        booking=b,
        context={"client_id": client.id, "service_code": service_code, "block_id": block_id, "price_cents": price_cents},
    )
...
def portal_book(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and request.POST.get("confirm") == "1":
            ...
            audit_emit(
                "booking.created.portal",
                message=f"Portal booking created by client {client.id}",
                actor=request.user,
                booking=b,
                context={"client_id": client.id, "service": svc.code, "start": str(start_dt), "end": str(end_dt)},
            )
