...
def create_bookings_with_billing(client: Client, rows: List[Dict]) -> Dict:
    ...
    with transaction.atomic():
        for row in rows:
            service_label_or_code = row.get('service_label', '')
            ...
            # Check if overnight service and adjust end_dt if needed
            if is_overnight(service_label_or_code):
                end_dt = end_dt + timedelta(days=1)
            
            price_cents = row.get('price_cents', 0)
            
            # Calculate credit usage for this booking
            ...
            # Add invoice item if this booking is being invoiced
            if booking_invoice_id and amount_due > 0:
                push_invoice_items_from_booking(booking, booking_invoice_id)
        
        # Deduct the total credit used from client
