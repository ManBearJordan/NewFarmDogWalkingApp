...
from PySide6.QtWidgets import (
    QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QTableWidget, QTableWidgetItem, QMessageBox, QTabWidget, QDateTimeEdit, QComboBox, QFileDialog, QSpinBox, QDateEdit, QTimeEdit, QCheckBox, QDialog, QMenu, QGroupBox)
...
class BookingsTab(QWidget):
    ...
    def refresh_two_weeks(self):
        ...
        sub_rows = []  # Bookings view excludes subscription holds
...
class CalendarTab(QWidget):
    ...
    def compute_subscription_holds_for_day(self, day_iso: str):
        c = self.conn.cursor()
        c.execute("""
            SELECT
                so.id,
                so.start_dt, so.end_dt, so.location, so.notes, so.dogs,
                so.stripe_subscription_id
            FROM sub_occurrences AS so
            WHERE so.active = 1
              AND date(so.start_dt) = ?
        ...
        for row in c.fetchall():
            ...
            holds.append({
                ...
                "start_dt": r["start_dt"],
                "end_dt": r["end_dt"],
                "dogs": r.get("dogs") or 1,
                "notes": r.get("notes") or "",
                "status": "hold",
                "source": "sub_occurrence",
            })
        return holds
...
class AdminTab(QWidget):
    def __init__(self, conn, parent=None):
        ...
        self._ensure_table(); self.refresh_google_status(); self.refresh_table()
    ...
    def refresh_table(self):
        c = self.conn.cursor()
        c.execute("SELECT id,title,due_date,completed FROM admin_events ORDER BY date(due_date) ASC, id DESC")
        rows = c.fetchall()
        self.table.setRowCount(0)
        for r in rows:
    ...
    def add_task(self):
        ...
        self.conn.execute("INSERT INTO admin_events(title,due_date,notes,completed) VALUES (?,?,?,0)", (title, due, notes))
...
class MainWindow(QMainWindow):
    ...
    def _show_admin_tasks_due_today(self):
        ...
        """)
        c.execute("SELECT id,title,COALESCE(notes,'') FROM admin_events WHERE completed=0 AND date(due_date)=?", (today,))
        rows = c.fetchall()
        if not rows: return
